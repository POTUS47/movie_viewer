<template>
    <div class="search-by-title">
        ...
        <div v-if="showDirectors && directors.length > 0" class="results-area">
            ...
            <div class="director-cards">
                <div v-for="(director, index) in directors" :key="index" class="director-card"
                    @click="showMoviesByDirector(director.genre_name)">
        ...
        <div v-if="!showDirectors && selectedMovies.length > 0" class="results-area">
            <h3> {{ currentDirector }}类型的电影：</h3>
            <div class="movie-cards">
                <div v-for="(movie, index) in selectedMovies" :key="index" class="movie-card"
                    @click="showMovieVersions(movie.movie_id)">
        ...
        <div v-else-if="!showDirectors && searchPerformed && !loading && selectedMovies.length === 0"
            class="no-results">
            <p>该类型没有电影记录。</p>
            <button @click="backToDirectors">返回</button>
        </div>
...
const showMoviesByDirector = (directorName) => {
    ...
    const apiUrl = `http://47.97.59.189:5000/search_by_genre/${directorName}`;
