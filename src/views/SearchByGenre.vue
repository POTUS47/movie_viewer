<template>
    <div class="search-by-title">
      ...
      <div class="search-container">
        <input v-model="searchQuery" type="text" placeholder="请输入类型名称..." @keyup.enter="performSearch" />
      ...
      <!-- 按钮区域，选择不同的类型查询方式 -->
      <div class="query-buttons" v-if="searchPerformed && genreVisible">
      ...
      <!-- 按钮区域，选择不同的电影查询方式 -->
      <div class="query-buttons" v-if="!genreVisible">
      ...
      <!-- 根据选中的查询方式展示不同的类型结果 -->
      <div class="results-area" v-if="genreVisible && selectedQuery && getSearchResults.length > 0">
        <h3>使用{{ selectedQuery }}搜索类型结果:{{ getSearchResults.length }}条</h3>
  
        <div class="movie-cards">
          <div v-for="(result, index) in getSearchResults" :key="index" class="movie-card"
            @click="searchMoviesByGenre(result.genre_name)">
      ...
      <!-- 根据选中的查询方式展示不同的电影结果 -->
      <div class="results-area" v-if="!genreVisible && selectedMovieQuery && getMovieSearchResults.length > 0">
        <h3> 使用{{ selectedMovieQuery }}搜索{{ currentGenre }} 类型电影结果：{{ getMovieSearchResults.length }}条</h3>
        <div class="movie-cards">
        ...
        <button @click="backToGenres" class="back-button">返回</button>
      </div>
  ...
  const mysqlSearchResults = ref([]);//类型搜索结果拆成三个
  ...
  const searchMoviesByGenre = (genreName) => {
    ...
    SqlSearchMoviesByGenre(genreName);
    Neo4jSearchMoviesByGenre(genreName);
    HiveSearchMoviesByGenre(genreName);
  }
  
  // 显示该类型的电影
  const SqlSearchMoviesByGenre = (genreName) => {
